<template>
    <section class="flex flex-col min-w-[520px] w-full">
        ...
        <div class="flex items-center gap-3.5 pb-4 border-b border-dashed">
            <div>
                <div class="w-12 h-12 overflow-hidden rounded-full image-fit border-2 border-slate-200/70">
                    <img src="public/fitness-alex.png" />
                </div>
            </div>
            <div>
        ...
        <!-- Messages -->
        <div class="text-center text-2xl py-12" v-if="isLoading">
...
import { assistant } from '~/server/utilities/openai';
...
const { data: apiMessages } = await useFetch("/api/message", {
    lazy: true,
    onResponse({ response }) {
