<template>
    <section class="flex flex-col min-w-[520px] w-full">
        ...
        <!-- Messages -->
        <div class="text-center text-2xl py-12" v-if="status === 'pending'">
            <svg xmlns="http://www.w3.org/2000/svg" 
                fill="none" 
                viewBox="0 0 24 24" 
                stroke-width="1.5" 
                stroke="currentColor" 
                class="w-12 h-12 mx-auto size-6 animate-spin">
                <path stroke-linecap="round" stroke-linejoin="round" d="M16.023 9.348h4.992v-.001M2.985 19.644v-4.992m0 0h4.992m-4.993 0 3.181 3.183a8.25 8.25 0 0 0 13.803-3.7M4.031 9.865a8.25 8.25 0 0 1 13.803-3.7l3.181 3.182m0-4.991v4.99" />
            </svg>

        </div>
        <div class="flex flex-col gap-3.5 py-5 px-3 overflow-y-scroll max-h-[400px]" v-else ref="container">
            <!-- To flip message use "flex-row-reverse" -->
            <div v-for="message in messages"
            class="flex items-end gap-3"
            :class="{ 'flex-row-reverse': !message.isAlex }">
                <!-- Profile Image -->
                <img
                v-if="message.isAlex"
                ...
                <div
                    ...
                    <!-- Message Content -->
                    <div v-if="message.isAlex" v-html="message.message"></div>
                    <div v-else>
                        {{ message.message }}
                    </div>
                    <!-- Time -->
        ...
        <MessageForm v-show="status !== 'pending'"/>
    </section>
</template>

<script setup lang="ts">

// Define the ThreadMessage  since it's not available in the OpenAI SDK
        ...
        if (response?._data?.data) {
            response._data.data.reverse().forEach((element: ThreadMessage) => {
                ...
                if (content?.type === "text") {
                    messages.value.push({
                        name: element.role === "assistant" ? "Alex" : customerInitials.value,
                        isAlex: element.role === "assistant",
