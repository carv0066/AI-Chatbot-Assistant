import { defineEventHandler, getQuery } from 'file://D:/Coding%20Projects/ai-chatbot-assistant/ai-chatbot-assistant/node_modules/h3/dist/index.mjs';
import { c as client, a as assistant } from '../../_/openai.mjs';
import 'file://D:/Coding%20Projects/ai-chatbot-assistant/ai-chatbot-assistant/node_modules/openai/index.mjs';

const thread = defineEventHandler(async (event) => {
  const queryParams = getQuery(event);
  const thread = await client.beta.threads.create();
  const run = await client.beta.threads.runs.create(thread.id, {
    assistant_id: assistant,
    additional_instructions: `The customer's name is ${queryParams.customer}`
  });
  return {
    thread: thread.id,
    run: run.id
  };
});

export { thread as default };
//# sourceMappingURL=thread.mjs.map
